...
x = residual_block(x, 64, 2, strides=2, dropout=DROPOUT)
x = residual_block(x, 128, 2, strides=2, dropout=DROPOUT)
out = residual_block(x, 256, 2, strides=1, dropout=DROPOUT)
convbase1 = keras.Model(convbase_inputs1, out, name="convbase1")
convbase1.summary()

convbase_inputs2 = keras.Input(shape=(8, 8, 256))
x = convbase_inputs2
x = residual_block(x, 320, 2, strides=2, dropout=DROPOUT)
out = residual_block(x, 512, 2, strides=1, dropout=DROPOUT)
convbase2 = keras.Model(convbase_inputs2, out, name="convbase2")
convbase2.summary()


encoder_inputs = keras.Input(shape=(4, 4, 512))
x = encoder_inputs
x = layers.Flatten()(x)
...
x = transpose_res_block(x, 512, 2, strides=2, dropout=DROPOUT)

x = transpose_res_block(x, 320, 2, strides=1, dropout=DROPOUT)

x = transpose_res_block(x, 256, 2, strides=1, dropout=DROPOUT)

x = transpose_res_block(x, 128, 2, strides=2, dropout=DROPOUT)

x = transpose_res_block(x, 64, 2, strides=2, dropout=DROPOUT)
