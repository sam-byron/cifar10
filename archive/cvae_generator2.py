...
x = transpose_res_block(x, 396, 2, strides=1, dropout=DROPOUT)

# x = transpose_res_block(x, 320, 2, strides=1, dropout=DROPOUT)

x = transpose_res_block(x, 256, 4, strides=2, dropout=DROPOUT)

x = transpose_res_block(x, 128, 4, strides=2, dropout=DROPOUT)
...
def augment_ds(images, labels, cvae):
    ...
    generated_imgs = noisy_decode(cvae, scaled_imgs)
...
if CLASSIFY:
    ...
    DROPOUT = 0.1
    x = layers.Dense(math.floor(1024*DIFFICULITY), kernel_initializer = initializer, use_bias=False)(x)
    x = layers.BatchNormalization()(x) 
    x = layers.Activation(ACTIVATION)(x)
