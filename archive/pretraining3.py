...
USE_PRETRAIN = True
CLASSIFY = True
VAE_LOCATION = '/home/sambyron/engineering/ML/tensorflow/cifar10/model_pretraining3_tf2.8/vae_weights'
...
x = layers.Dense(16 * 16 * 256, activation="relu", kernel_initializer=initializer)(latent_inputs)
x = layers.Reshape((16, 16, 256))(x)
x = layers.Conv2DTranspose(256, 3, activation="relu", strides=2, padding="same", kernel_initializer=initializer)(x)
# x = layers.SpatialDropout2D(DROPOUT)(x)
# x = layers.BatchNormalization()(x)
...
class VAE(keras.Model):
    ...
    def train_step(self, data):
        with tf.GradientTape() as tape:
            ...
            total_loss = reconstruction_loss + 7*tf.reduce_mean(kl_loss)
...
if CLASSIFY:
    ...
    lr_schedule = optimizers.schedules.ExponentialDecay( initial_learning_rate=INIT_LEARNING_RATE, decay_steps=10000, decay_rate=DECAY_RATE)
    ...
    if USE_PRETRAIN:
        i = 1
        cnn.layers[1+i].set_weights(vae.encoder.layers[1].get_weights())
        cnn.layers[2+i].set_weights(vae.encoder.layers[2].get_weights())
        cnn.layers[3+i].set_weights(vae.encoder.layers[3].get_weights())
        cnn.layers[4+i].set_weights(vae.encoder.layers[4].get_weights())
        cnn.layers[5+i].set_weights(vae.encoder.layers[5].get_weights())
        cnn.layers[6+i].set_weights(vae.encoder.layers[6].get_weights())
