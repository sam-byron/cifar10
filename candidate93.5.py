...
x = convbase_inputs1
x = residual_block(x, 32, 3, strides=2, dropout=DROPOUT)
out = residual_block(x, 128, 4, strides=2, dropout=DROPOUT)
convbase1 = keras.Model(convbase_inputs1, out, name="convbase1")
convbase1.summary()

convbase_inputs2 = keras.Input(shape=(8, 8, 166))
x = convbase_inputs2
out = residual_block(x, 256, 4, strides=2, dropout=DROPOUT)
convbase2 = keras.Model(convbase_inputs2, out, name="convbase2")
convbase2.summary()


encoder_inputs = keras.Input(shape=(4, 4, 332))
x = encoder_inputs
x = layers.Flatten()(x)
...
x = layers.Dense(4 * 4 * 256, activation="relu", kernel_initializer=initializer)(latent_inputs)
x = layers.Reshape((4, 4, 256))(x)

x = transpose_res_block(x, 256+MODIFIER, 4, strides=2, dropout=DROPOUT)

x = transpose_res_block(x, 128+MODIFIER, 4, strides=2, dropout=DROPOUT)

x = transpose_res_block(x, 64+MODIFIER, 3, strides=2, dropout=DROPOUT)
