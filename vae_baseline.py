...
x = layers.Dense(16 * 16 * 512, activation="relu", kernel_initializer=initializer)(latent_inputs)
...
if CLASSIFY:
    ...
    # residual = x
    # x = layers.Concatenate(axis=3)([x, residual])
    x = layers.Conv2D(768*DIFFICULITY, 3, kernel_initializer = initializer, padding="same", use_bias=False)(x)
    x = layers.BatchNormalization()(x) 
    x = layers.Activation(ACTIVATION)(x)
    # x = layers.Concatenate(axis=3)([x, residual])
    x = layers.SpatialDropout2D(DROPOUT)(x)
    ...
    x = layers.Flatten()(x)
    if USE_PRETRAIN:
        x = layers.Concatenate()([x, z_mean, z_log_var])
